...
def _run_uninstall() -> None:
    ...
    for cmd in ("ping!", "tcp!", "bgp!", "trace!", "http!", "info!",
                ...
                "ff", "fc", "fcheck"):
        path = _os.path.join(scripts, cmd)
...
def _bot_settings() -> None:
    ...
    while True:
        ...
        if sub == "0":
            ...
            print(f"  {'─' * w}")
            print(f"  {B}1.{N}  Open Telegram and message {C}@BotFather{N}")
            print(f"  {B}2.{N}  Send {C}/newbot{N} and follow the prompts")
            print(f"  {B}3.{N}  Copy the API token it gives you")
            print(f"  {B}4.{N}  Here:  option {B}1{N}  →  paste the token")
            print(f"  {B}5.{N}  (Optional) option {B}4{N}  →  restrict to your")
            print(f"      own chat ID  {DIM}(private mode){N}")
            print(f"  {B}6.{N}  Start it:  option {B}3{N}  {DIM}(this terminal){N}")
            print(f"      or option {B}5{N}  {DIM}(systemd — always on){N}")
            print()
            print(f"  {DIM}Then, in your bot on Telegram:{N}")
            print(f"    {C}/start{N}   opens the menu — everything is buttons:")
            print(f"      {DIM}·{N}  Add IP / Domain, then Check All")
            print(f"      {DIM}·{N}  set an auto-check interval")
            print(f"      {DIM}·{N}  Domain Check  {DIM}(availability & WHOIS){N}")
            print(f"      {DIM}·{N}  pause / resume monitoring")
            print()
            print(f"  {DIM}Find your chat ID via {C}@userinfobot{N}{DIM}.{N}")
            print(f"  {'─' * w}")
