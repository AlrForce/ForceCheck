...
def _help_text() -> str:
    return (
        ...
        f"<i>ForceCheck pings your IPs & domains from 100+ probe\n"
        f"nodes — Iranian and international — and tells you whether\n"
        f"each one is reachable inside Iran, worldwide, or blocked.\n"
        f"It can also watch them on a schedule and alert you.</i>\n\n"

        f"{_DIV}\n"
        f"<b>BUTTONS</b>\n\n"

        f"{E_SEARCH}  <b>Check All Now</b>\n"
        f"  Runs a check right now. First pick a single target for\n"
        f"  a live, node-by-node ping, or “Check All” for a quick\n"
        f"  summary of every target on your list.\n\n"

        f"{E_SAT}  <b>My List</b>\n"
        f"  Shows every IP / domain you're watching, the interval,\n"
        f"  and whether monitoring is active.\n\n"

        f"{E_ADD}  <b>Add IP / Domain</b>\n"
        f"  Adds an IPv4 address or a domain to your watch list.\n"
        f"  <i>e.g.  1.2.3.4  or  example.com   ·   up to 20 targets</i>\n\n"

        f"{E_TRASH}  <b>Remove</b>\n"
        f"  Removes a target — tap the one you want gone.\n\n"

        f"{E_CLOCK}  <b>Set Interval</b>\n"
        f"  How often the bot auto-checks your list in the\n"
        f"  background and sends you the results.\n"
        f"  <i>Minimum 5 min  ·  recommended 30 – 60 min</i>\n\n"

        f"{E_BELL}  <b>Alerts</b>  <i>(chosen on the interval screen)</i>\n"
        f"  <b>Always</b> — send every scheduled report.\n"
        f"  <b>Only when Iran Access</b> — stay silent unless a\n"
        f"  target is reachable from Iran but blocked elsewhere.\n\n"

        f"{E_PAUSE}  <b>Pause</b>  /  {E_PLAY}  <b>Resume</b>\n"
        f"  Turns the scheduled auto-checks off / on.\n"
        f"  <i>“Check All Now” still works while paused.</i>\n\n"
        ...
        f"answer:  ≥ 2 Iranian  and / or  ≥ 5 global.</i>\n\n"

        f"  {E_OK}  <b>Globally Accessible</b>\n"
        f"  <i>Works from both Iran and abroad — no blocking.</i>\n\n"

        f"  {E_RED}  <b>Iran Access Only</b>\n"
        f"  <i>Answers inside Iran but not abroad — e.g. an\n"
        f"  Iran-only server, or blocked/sanctioned globally.</i>\n\n"

        f"  {E_WARN}  <b>Restricted · Filtered</b>\n"
        f"  <i>Works abroad but blocked inside Iran — i.e. the\n"
        f"  target is filtered by Iran.</i>\n\n"

        f"  {E_ERR}  <b>Host Unreachable</b>\n"
        f"  <i>No node anywhere got a reply — down or fully blocked.</i>\n\n"
